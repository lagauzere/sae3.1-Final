...
<head>
    ...
    <title>Liste des plong?es disponibles</title>
    @livewireStyles
    <!-- Styles -->
...
<body>
    <x-header />
    <h1>Liste des plong?es disponibles</h1>
    <?php
   ...
   <script src='https://cdn.jsdelivr.net/npm/fullcalendar@5.6.0/main.min.js'></script>
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
                        ...
                        if(info.event.extendedProps.levelId <= DiversLevel[0].DLV_ID && info.event.extendedProps.remainingCapacity > 0) {
                            content.innerHTML = `
                                <div style="cursor: pointer;">
                                    <strong>Plong?e num?ro:  ${info.event.title}</strong><br>
                    ...
                    eventClick: function(info) {
                        ...
                        var modalContent = `
                            <div class="modal-header">
                                <h5 class="modal-title"> Plong?e num?ro: ${info.event.title} </h5>
                        ...
                        divesForDivers.forEach(dive => {
                            if(dive.DIV_ID == info.event.title && dive.PAR_CANCELLED == 0){
                                modalContent += `
                                        ...
                                        @csrf 
                                        <button type="submit" class="btn btn-primary" data-dismiss="modal">Se d?sinscrire</button>
                                        </form>`
                                ...
                                cancelled = true;
                                registered = true;
                                modalContent += `<p style= "color:red" > Vous avez d?j? annul? la participation ? cette plong?e</p>`
                            }
                        })
                        if(registered==false && cancelled == false ){
                            ...
                            modalContent += `<form id="registerForm" action="" method="POST">
                                    @csrf <button type="submit" class="btn btn-primary" disabledS'inscrire</button </form`
                                document.getElementById('dynamic-modal-content').innerHTML = modalContent; 
                                document.getElementById('registerForm').action = registerFormAction.replace(':selectedDive', info.event.title);
                        }
        ...
        <script ="https://cdn.jsdelivr.net/npm/fullcalendar@5.6.0/locales-all.min.js"></script>
